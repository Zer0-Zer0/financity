...
namespace Economy
{
    public class CandlestickChart : MonoBehaviour
    {
        ...
        public float lowest = -1f,
            highest = -1f;
        public float maxCandleHeight = 160f,
            maxCandleY = 200f;
        ...
        void RefreshCandleScale()
        {
            ...
            foreach (CandleRender candle in candles)
            {
                ...
                candleTransform.sizeDelta = new Vector2(
                    candleTransform.sizeDelta.x,
                    maxCandleHeight * candleRatio
                );

                //handling the candle Y position

                float candleYRatio =
                    (((candle.min + candle.max) / 2) - lowest) / (highest - lowest);
                candleTransform.anchoredPosition = new Vector2(
                    candleTransform.anchoredPosition.x,
                    candleTransform.anchoredPosition.y + candleYRatio * maxCandleY
                );
            }
        }
        ...
        {
            CandleRender newCandle = Instantiate(
                candlePrefab,
                transform.position,
                transform.rotation
            );
        ...
        public void HandleStockUpdate(Component sender, object data)
        {
            if (lowest > low)
            {
                lowest = low;
            ...
            Debug.Log(
                $"Received stock update - Open: ${open}, Close: ${close}, HighShadow: ${high}, LowShadow: ${low}"
            );
